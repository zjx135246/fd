/*
今日头条极速版
IOS/安卓: 今日头条极速版     //xzf只留了1、分享2、读书3、农场
邀请码： CYET6KHK
老用户每天几毛，新用户可能收益高点
普通版定时： 1-59/15 6-23 * * *
激进版定时： 1-59/5 * * * *
多用户跑的时间会久一点，自己看着改定时吧
export jrttjsbHeader='cookie全部'
export jrttjsbUA=''         // 自定义UA：捉包拿到自己的UA，填到变量jrttjsbUA里，不填默认用安卓UA
export jrttjsbReadNum='10' //自定义每次运行阅读文章的数量：填到变量jrttjsbReadNum，不填默认10篇
export jrttjsbFarm='1'     //农场和种树任务：默认做，变量jrttjsbFarm填为 1 做 0 不做
...
async function RunMultiUser() {
    for(userIdx=0; userIdx<userHeaderArr.length; userIdx++) {
        ...
        if(userStatus[userIdx]==true) {
            await GetNewTabs()
            //await QuerySleepStatus() //xzf睡觉
           // await QueryWalkInfo() //xzf走路
            //await DoneEat()  //xzf吃饭

           // for(let adId of adIdList) await ExcitationAd(adId) //看视频，领宝箱
...
async function ReadArticles() {
    ...
    for(userIdx=0; userIdx<userHeaderArr.length; userIdx++) {
        if(userStatus[userIdx]==true) {
            await ReadDouble()   //xzf阅读翻倍
            await DailyArtsReward() //xzf每日阅读奖励
    ...
    for(let i=0; i<maxReadPerRun; i++) {
        ...
        for(userIdx=0; userIdx<userHeaderArr.length; userIdx++) {
            if(userStatus[userIdx]==true) {
               // await ReadArtsReward() //xzf阅读文章奖励
                readFlag = 1
            }
        }
